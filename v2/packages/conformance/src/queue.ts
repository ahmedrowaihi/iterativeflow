...
export const queueConformance = (label: string, makeQueue: () => Queue | Promise<Queue>): void => {
  describe(`Queue conformance — ${label}`, () => {
    it("enqueue then claim returns the run with a lease", async () => {
      ...
      await q.enqueue("r1");
      const leases = await q.claim({ max: 10, leaseMs: 1000, now: at(0) });
    ...
    it("a future runAt is not claimable until due", async () => {
      ...
      expect(await q.claim({ max: 10, leaseMs: 1000, now: at(0) })).toEqual([]);
      const due = await q.claim({ max: 10, leaseMs: 1000, now: at(5000) });
    ...
    it("a claimed run is leased exclusively — a second claim before expiry gets nothing", async () => {
      const q = await makeQueue();
      await q.enqueue("r1");
      const first = await q.claim({ max: 10, leaseMs: 1000, now: at(0) });
      expect(first).toHaveLength(1);
      const second = await q.claim({ max: 10, leaseMs: 1000, now: at(500) });
    ...
    it("an expired lease is re-claimable (crash recovery), with a fresh token", async () => {
      ...
      await q.enqueue("r1");
      const first = await q.claim({ max: 10, leaseMs: 1000, now: at(0) });
      const reclaim = await q.claim({ max: 10, leaseMs: 1000, now: at(1001) });
    ...
    it("heartbeat extends the lease so it isn't re-claimed at the original expiry", async () => {
      ...
      expect(await q.claim({ max: 10, leaseMs: 1000, now: at(1001) })).toEqual([]);
      const after = await q.claim({ max: 10, leaseMs: 1000, now: at(1501) });
    ...
    it("ack removes the run so it isn't claimed again", async () => {
      ...
      expect(await q.claim({ max: 10, leaseMs: 1000, now: at(2000) })).toEqual([]);
    });
    ...
    it("a stale ack is a no-op — it does not delete the run the new owner holds", async () => {
      ...
      await q.enqueue("r1");
      const [stale] = await q.claim({ max: 10, leaseMs: 1000, now: at(0) });
      await q.claim({ max: 10, leaseMs: 1000, now: at(1001) }); // re-claim
      await q.ack(stale, { now: at(1002) }); // stale token — must not remove
      const still = await q.claim({ max: 10, leaseMs: 1000, now: at(2002) });
    ...
    it("a wake that re-enqueues during a lease survives the ack — the job is released, not deleted", async () => {
      ...
      const again = await q.claim({ max: 10, leaseMs: 1000, now: at(2) });
    ...
    it("an expired ack is a no-op — an expired owner can't delete a re-claimable run", async () => {
      const q = await makeQueue();
      await q.enqueue("r1");
      const [lease] = await q.claim({ max: 10, leaseMs: 1000, now: at(0) }); // expires at(1000)
      await q.ack(lease, { now: at(1001) }); // expired → CAS fails → no-op
      const still = await q.claim({ max: 10, leaseMs: 1000, now: at(1002) });
      expect(still.map((l) => l.runId)).toEqual(["r1"]);
    });
    ...
    it("re-enqueue does not yank an active lease from the current worker", async () => {
      const q = await makeQueue();
      await q.enqueue("r1");
      await q.claim({ max: 10, leaseMs: 1000, now: at(0) }); // leased until at(1000)
    ...
    it("claim honours the batch max", async () => {
      ...
      const leases = await q.claim({ max: 2, leaseMs: 1000, now: at(0) });
    ...
    it("lower priority is claimed first", async () => {
      ...
      const [first] = await q.claim({ max: 1, leaseMs: 1000, now: at(0) });
